#!/bin/bash
# Run Pedestrian Client with secrets from .env
set -e
...
if [ -z "$CLIENT_SECRET" ]; then
    echo "❌ Error: PEDESTRIAN_CLIENT_SECRET not found in .env"
    exit 1
fi

# Build
echo "Building Pedestrian Client..."
mvn clean package -DskipTests

# Run
echo "Starting Pedestrian Client..."
