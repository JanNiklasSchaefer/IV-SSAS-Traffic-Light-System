...
public class OtherVehicleClient implements QuarkusApplication {
                ...
                try {
                    switch (command) {
                        ...
                        case "2" -> {
                            if (parts.length != 2) {
                                System.out.println(
                                        "ERROR: Missing traffic-light-id or too many Input Parameters. Correct Pattern: 1 <Boolean>. Accepted Values: true, false. Example Call: \n> 1 true");
                            } else {
                        ...
                        case "3" -> {
                            if (parts.length != 3) {
                                ...
                                callPriorityRequestEndpoint(UUID.fromString(parts[1]), UUID.fromString(parts[2]));
                            }
                        }
                        case "4" -> {
                            if (parts.length != 2) {
    ...
    private void printMenu() {
        System.out.println(
                """
                        ...
                        3 <trafficLightId> <vehicleId>    → POST /api/priority/requests Body: {"trafficLightId" : {UUID}, "vehicleId" : {UUID} , "vehicleType" : "other"}
    ...
    private void runDemo() throws Exception {
        ...
        callPriorityRequestEndpoint(TRAFFIC_LIGHT_DUMMY_UUID, UUID.randomUUID());
    ...
    private void callPriorityRequestEndpoint(UUID trafficLightId, UUID vehicleId) throws Exception {
        System.out.println("\n--- POST /api/priority/requests ---");
